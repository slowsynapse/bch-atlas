...
If running `claude` directly in this repo:
- Hooks defined in `.claude/settings.json` auto-apply
- Dangerous bash commands blocked pre-execution
- Prettier auto-formats on edit

### Via OpenClaw ACP Runtime
If spawned via `sessions_spawn(runtime: "acp")`:
- Read this AGENTS.md first
- Read `.tasks/active-work.md` for current state
- Respect `.acp/config.toml` permissions
